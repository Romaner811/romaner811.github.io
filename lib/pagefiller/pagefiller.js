...
class GamePage {
    ...
    static getCollabLinkRef(id, link_type) {
        let available_links = collabs[id].link;
        let ref = null;
        
        if ((link_type == LINKTYPE_ANY) || (link_type == null)) {
            if (collabs[id].hasOwnProperty("default_link_type")) {
                ref = available_links[collabs[id].default_link_type];
            } else {
                for (let i = 0; i < LINK_PRIORITY.length; i++) {
                    ref = available_links[LINK_PRIORITY[i]];
                    if (ref != null) {
                        break;
                    }
                }
    ...
    static generateTeamMemberHTML(username, info) {
        let ref = GamePage.getCollabLinkRef(username);
        
        return `<a href="${ref}" target="_blank">${info.id}</a>`;
    }
    ...
    fillTemplate(jq_elem) {
        ...
        this._fillOrHide(jq_elem.find(".description"), this.description);
        this._fillOrHide(jq_elem.find(".work"), this.work);
        this._fillOrHide(jq_elem.find(".extra"), this.extra);
    }
    
    _fillOrHide(jq_elem, content) {
        if (content != null)
        {
            jq_elem.html(content);
        }
        else
        {
            jq_elem.hide();
